"""
Script principal para extraer metadatos de SQL Server y generar documentación.
"""
import yaml
import os
import sys
...
from services.mssql_metadata_service import MSSQLMetadataService
...
def main():
    """Función principal"""
    setup_logging()
    log_start("📊 EXTRACCIÓN DE METADATOS - SQL SERVER")
    
    start_time = datetime.now()
    
    try:
        # Cargar configuración
        config_path = "config-mssql.yaml"
        log_info(f"📋 Cargando configuración: {config_path}")
        config = load_config(config_path)
        
        # Crear servicio de metadatos
        log_info("🔗 Conectando a SQL Server...")
        metadata_service = MSSQLMetadataService(config['database'])
