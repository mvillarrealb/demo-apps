...
├── main.py                   # ⭐ Script unificado (RECOMENDADO)
├── main_postgres_metadata.py  # Script legacy PostgreSQL
├── main_mssql_metadata.py    # Script legacy SQL Server
├── main_oracle_metadata.py   # Script legacy Oracle
...
Ahora puedes usar un único entrypoint que recibe el vendor como argumento:

**Con uv (Recomendado):**
```bash
# Para PostgreSQL
uv run main.py --vendor postgres

# Para SQL Server
uv run main.py --vendor mssql

# Para Oracle
uv run main.py --vendor oracle

# O usando el comando instalado (después de uv sync):
uv run db-metadata --vendor postgres
uv run db-metadata --vendor mssql
uv run db-metadata --vendor oracle
```
...
db-metadata --vendor postgres
db-metadata --vendor mssql
db-metadata --vendor oracle
```
...
#### Método Legacy (scripts separados)

Los scripts individuales aún están disponibles para compatibilidad:
...
# O con comandos legacy:
uv run db-metadata-postgres
uv run db-metadata-mssql
uv run db-metadata-oracle
```
